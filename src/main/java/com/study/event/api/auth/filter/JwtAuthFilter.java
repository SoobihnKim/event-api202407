...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            log.info("토큰 위조 검사 필터 작동 JWT token: {}", token);
            if (token != null) {
                // 토큰 위조 검사
                ...
                AbstractAuthenticationToken auth
                        = new UsernamePasswordAuthenticationToken(tokenInfo,  // 인증 완료 후 컨트롤러에서 사용할 정보
                        null, // 인증된 사용자의 패스워드 - 보통 null로 둠
                        new ArrayList<>()); // 인가정보(권한) 리스트

                // 인증 완료 시 클리이언트의 요청 정보들을 세팅
                auth.setDetails(
        ...
        // 필터 체인: 필터는 여러개임. 우리가 체인에 걸어놓은 필터를 실행 명령
        // 등록은 시큐리티에서 해야함
    ...
    private String parseBearerToken(HttpServletRequest request) {
        /*
          ...
          {
            'Authorization' : 'Bearer dfsfsfsdfa',
